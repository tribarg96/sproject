...
app.post('/api/register', async (req, res) => {
  ...
  const { data, error } = await supabase.from('users').insert({
    username, password_hash: hash, role: role || 'student', display_name: displayName || username, avatar_color: color
  }).select().single();
...
app.post('/api/assignments', async (req, res) => {
  ...
  const { data, error } = await supabase.from('assignments').insert({ title, description, subject, due_date, created_by }).select().single();
...
app.post('/api/grades', async (req, res) => {
  ...
  const { data, error } = await supabase.from('grades').insert({ student_username, subject, grade, comment }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
...
app.post('/api/homework', async (req, res) => {
  ...
  const { data, error } = await supabase.from('homework_submissions').insert({ assignment_id, student_username, student_display_name, content }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
...
app.patch('/api/homework/:id', async (req, res) => {
  ...
  const { data, error } = await supabase.from('homework_submissions').update({ status }).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
