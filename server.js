...
app.post('/api/register', async (req, res) => {
  ...
  // Кодируем пароль для корректной работы с кириллицей
...
app.post('/api/assignments', async (req, res) => {
  ...
  const { data } = await supabase.from('assignments').insert({ 
    title, description, subject, due_date, created_by 
  }).select().single();
  res.json(data);
});
...
app.post('/api/grades', async (req, res) => {
  ...
  const { data } = await supabase.from('grades').insert({ 
    student_username, subject, grade, comment 
  }).select().single();
  res.json(data);
});
...
app.post('/api/homework', async (req, res) => {
  ...
  const { data } = await supabase.from('homework_submissions').insert({ 
    assignment_id, student_username, student_display_name, content 
  }).select().single();
  res.json(data);
});
...
io.on('connection', (socket) => {
  ...
  socket.on('send_message', async (data) => {
    ...
    const { data: msg } = await supabase.from('messages').insert({ 
      sender_username, sender_display_name, content 
    }).select().single();
